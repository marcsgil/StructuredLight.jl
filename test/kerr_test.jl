...
function calculate_projections(rs, zs, g, l; kwargs...)
    ψ₀ = lg(rs, rs; l, kwargs...)
    ...
    for p in axes(numerical_c, 2)
        corrected_ψ = lg(rs, rs, zs, p=p - 1, l=l, w=1 / √3, k=2, kwargs...)
...
function run_kerr_tests(rs, zs, g, l, identifier=""; kwargs...)
...
if CUDA.functional()
    ...
    run_kerr_tests(rs, zs, g, l, " (CUDA)"; backend=CUDABackend())
end